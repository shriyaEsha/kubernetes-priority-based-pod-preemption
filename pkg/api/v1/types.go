...
type PodSpec struct {
	...
	// Priority assigned to each pod to enable preemptive scheduling and eviction of pods
	// +optional
	Priority uint8 `json:"priority,omitempty" protobuf:"bytes,24,opt,name=priority"`

	// EvictionCost - cost of removing pod from node
	// Used to determine whether pod needs to be preempted or not when scheduling new pods
	// +optional
	EvictionCost uint8 `json:"evictionCost,omitempty" protobuf:"bytes,25,opt,name=evictionCost"`
