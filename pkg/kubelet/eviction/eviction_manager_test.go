...
// pods with priority
func makePodWithMemoryAndPriorityStats(priority uint8, name string, requests v1.ResourceList, limits v1.ResourceList, memoryWorkingSet string) (*v1.Pod, statsapi.PodStats) {
	pod := newPod(name,  []v1.Container{
		newContainer(name, requests, limits),
	}, nil)
...
// with priority
type podToMakeWithPriority struct {
	priority uint8
	name                     string
	requests                 v1.ResourceList
	limits                   v1.ResourceList
...
// TestMemoryPressure 
func TestMemoryPressureWithPriority(t *testing.T) {
	podMaker := makePodWithMemoryAndPriorityStats
	summaryStatsMaker := makeMemoryStats
	podsToMake := []podToMakeWithPriority{
		{priority: 6, name: "guaranteed-low", requests: newResourceList("100m", "1Gi"), limits: newResourceList("100m", "1Gi"), memoryWorkingSet: "200Mi"},
		{priority: 7, name: "guaranteed-high", requests: newResourceList("100m", "1Gi"), limits: newResourceList("100m", "1Gi"), memoryWorkingSet: "800Mi"},
		{priority: 8, name: "burstable-low", requests: newResourceList("100m", "100Mi"), limits: newResourceList("200m", "1Gi"), memoryWorkingSet: "300Mi"},
		{priority: 9, name: "burstable-high", requests: newResourceList("100m", "100Mi"), limits: newResourceList("200m", "1Gi"), memoryWorkingSet: "800Mi"},
		{priority: 10, name: "best-effort-low", requests: newResourceList("", ""), limits: newResourceList("", ""), memoryWorkingSet: "300Mi"},
		{priority: 7, name: "best-effort-high", requests: newResourceList("", ""), limits: newResourceList("", ""), memoryWorkingSet: "500Mi"},
	}
	pods := []*v1.Pod{}
	podStats := map[*v1.Pod]statsapi.PodStats{}
	for _, podToMake := range podsToMake {
		pod, podStat := podMaker(podToMake.priority ,podToMake.name, podToMake.requests, podToMake.limits, podToMake.memoryWorkingSet)
		pods = append(pods, pod)
		podStats[pod] = podStat
	}
	podToEvict := pods[4]
	...
	// create a best effort pod to test admission
	bestEffortPodToAdmit, _ := podMaker(6, "best-admit", newResourceList("", ""), newResourceList("", ""), "0Gi")
	burstablePodToAdmit, _ := podMaker(6, "burst-admit", newResourceList("100m", "100Mi"), newResourceList("200m", "200Mi"), "0Gi")
