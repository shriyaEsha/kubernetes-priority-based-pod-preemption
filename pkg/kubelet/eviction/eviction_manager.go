...
func (m *managerImpl) synchronize(diskInfoProvider DiskInfoProvider, podFunc ActivePodsFunc, nodeProvider NodeProvider) {
	...
	fmt.Println("Before stablesort: ", format.Pods(activePods))
	// fmt.Println("ac[0]: ", activePods[0].Spec.Priority)

	// activePods[0].Spec.Priority = 8
	// fmt.Println("ac[0]: ", activePods[0].Spec.Priority)
	// stable sort of pods by priority
	var priorityFlag = true
	fmt.Println("PriorityFlag:", priorityFlag)
	glog.Infof("Stable sort of active Pods by Priority!")
	if priorityFlag == true{
		sort.SliceStable(activePods, func(i, j int) bool { 

			if activePods[i].Spec.Priority == activePods[j].Spec.Priority{
				return activePods[i].Spec.EvictionCost < activePods[j].Spec.EvictionCost
			}

			return activePods[i].Spec.Priority > activePods[j].Spec.Priority })

		glog.Infof("By priority: %s", format.Pods(activePods))
		fmt.Println("By Priority!", format.Pods(activePods))
	}
